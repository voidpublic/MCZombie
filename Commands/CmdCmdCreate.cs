...
namespace MCForge.Commands
{
    public class CmdCmdCreate : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (name.Length == 1)
            {
                if (File.Exists("extra/commands/source/Cmd" + message + ".cs")) { Player.SendMessage(p,"File Cmd" + message + ".cs already exists.  Choose another name."); return; }
                try
                {
            ...
            if (name[1] == "vb")
            {
                if (File.Exists("extra/commands/source/Cmd" + name[0] + ".vb")) { Player.SendMessage(p,"File Cmd" + name[0] + ".vb already exists.  Choose another name."); return; }
                try
                {
