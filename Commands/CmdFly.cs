...
namespace MCForge.Commands
{
    public class CmdFly : Command
    {
                            ...
                            try
                            {
                                for (ushort xx = (ushort)(x - 1); xx <= x + 1; xx++)
                                {
                                    for (ushort yy = (ushort)(y - 1); yy <= y; yy++)
                                    {
                                        ...
                                        {
                                            if (p.level.GetTile(xx, yy, zz) == Block.air || p.level.GetTile(xx,yy,zz) == Block.op_air)
                                            {
        ...
        {
            Player.SendMessage(p, "/fly - Allows you to fly, only when your referee");
        }
