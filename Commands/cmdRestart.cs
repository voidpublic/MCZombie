...
namespace MCForge.Commands
{
    public class CmdRestart : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (!Server.zombieRound && !p.saferestart)
            {
                Player.SendMessage(p, c.red + "Cannot restart while round is preparing");
                Player.SendMessage(p, c.red + "If you want to force restart type /restart again");
                p.saferestart = true;
                return;
            }
            MCForge_.Gui.Program.ExitProgram(true);
