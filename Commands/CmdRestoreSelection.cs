    ...
    class CmdRestoreSelection : Command
    {
            ...
            if (message != "")
                if (File.Exists(@Server.backupLocation + "/" + p.level.name + "/" + message + "/" + p.level.name + ".lvl"))
                {
                    try
                    {
                        ...
                        Player.SendMessage(p,"Select two corners for restore.");
                    }
        ...
        public void Blockchange2(Player p, ushort x, ushort y, ushort z, byte type)
        {
            ...
            try
            {
                ...
                if (blocks.Length != p.level.blocks.Length) { Player.SendMessage(p,"Cant restore selection of different size maps."); blocks = null; return; }
