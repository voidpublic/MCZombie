...
namespace MCForge.Commands
{
    public class CmdReplace : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (!Block.canPlace(p, type) && !Block.BuildIn(type) && !p.referee) { Player.SendMessage(p, "Cannot replace that."); wait = 1; return; }

            if (!Block.canPlace(p, type2) && !p.referee) { Player.SendMessage(p, "Cannot place that."); wait = 1; return; }
