    ...
    public class CmdCountdown : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            else if ((int)p.group.Permission >= CommandOtherPerms.GetPerm(this, 2))
            {
                    ...
                    if (CountdownGame.gamestatus == CountdownGameStatus.Enabled)
                    {
                        if (CountdownGame.players.Count >= 2)
                        {
                            ...
                            switch (par1)
                            {
                                ...
                                default:
                                    Player.SendMessage(p,"You didn't specify a speed, resorting to 'normal'");
                ...
                {
                    Player.SendMessage(p,"First, download the map using /countdown download");
                    Player.SendMessage(p,"Next, type /countdown enable to enable the game mode");
                    Player.SendMessage(p,"Next, type /countdown join to join the game and tell other players to join aswell");
                    Player.SendMessage(p,"When some people have joined, type /countdown start [speed] to start it");
                    Player.SendMessage(p,"[speed] can be 'ultimate', 'extreme', 'fast', 'normal' or 'slow'");
                    Player.SendMessage(p,"When you are done, type /countdown reset [map/all]");
                    Player.SendMessage(p,"use map to reset only the map and all to reset everything.");
                    return;
                }
            }
            else
            {
                Player.SendMessage(p,"Sorry, you aren't a high enough rank or that wasn't a correct command addition.");
                return;
            }
        }
        public override void Help(Player p)
        {
            ...
            Player.SendMessage(p,"/countdown players - view players currently playing");
            {
                ...
                {
                    Player.SendMessage(p,"/countdown rules - view the rules of countdown");
                }
