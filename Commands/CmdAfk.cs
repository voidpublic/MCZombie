            ...
            if (p != null)
            {
                if (message != "list")
                {
                    ...
                    if (!Server.afkset.Contains(p.name))
                    {
                        ...
                        if (!p.voice && (Server.votingforlevel || Server.chatmod || Server.voting))
                        {
                            message = "";
                        }
                        if(!p.muted && !p.hidden) Player.GlobalMessage("-" + p.color + p.name + Server.DefaultColor + "- is AFK " + message);
                    ...
                    else
                    {
                        ...
                        p.autoafk = false;
                        if(!p.muted && !p.hidden) Player.GlobalMessage("-" + p.color + p.name + Server.DefaultColor + "- is no longer AFK");
