...
namespace MCForge.Commands
{
	public class CmdGifToCin : Command
	{
		...
		public override void Use(Player p, string message)
		{
            ...
            {
                Player.SendMessage(p,"Place 2 Blocks to Determine the Direction");
                p.ClearBlockchange();
        ...
        public void Blockchange2(Player p, ushort x, ushort y, ushort z, byte Type)
        {
            ...
            {
                direction = 1;
                Player.SendMessage(p,"dir1");
            }
            else if (blox[1].x < blox[0].x)
            {
                direction = 3;
                Player.SendMessage(p,"dir3");
            }
            else if (blox[1].z > blox[0].z)
            {
                direction = 2;
                Player.SendMessage(p,"dir2");
            }
            else if (blox[1].z < blox[0].z)
            {
                direction = 4;
                Player.SendMessage(p,"dir4");
            }
            else
            {
                direction = 1;
                Player.SendMessage(p,"else");
            }
			...
			using (Stream imageStreamSource = new FileStream("extra/images/" + msg + ".gif", FileMode.Open, FileAccess.Read, FileShare.Read))
			{
				...
				using (System.Drawing.Bitmap tbmp = new System.Drawing.Bitmap("extra/images/0.bmp"))
				{
					...
					for (int i = 0; i < decoder.Frames.Count; i++)
					{
						Player.SendMessage(p,"Start processing Frame " + i);
						workFrame(i, p, cinName, direction);
						Player.SendMessage(p,"Done");
					}
					Player.SendMessage(p,"YAY! everything should be done");
