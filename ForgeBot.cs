...
namespace MCForge
{
    public class ForgeBot
    {
        ...
        private List<string> allowedCmd;
        ...
        public ForgeBot(string channel, string opchannel, string nick, string server)
        {
            ...
            if (Server.irc)
            {
                ...
                {
                    if (!File.Exists("text/irccmdallowed.txt")) File.WriteAllLines("text/irccmdallowed.txt", new String[] { "#Here you can put commands that can be used from the IRC bot.", "#Lines starting with \"#\" are ignored." });
                    foreach (string line in File.ReadAllLines("text/irccmdallowed.txt"))
                        if (line[0] != '#') allowedCmd.Add(line);
                }
            }
        }
        ...
        void Listener_OnPrivate(UserInfo user, string message)
        {
            ...
            if (!allowedCmd.Contains(message.Split(' ')[0])) { Pm(user.Nick, "You cannot use this command from IRC!"); return; }
        ...
        {
            if (Server.irc && IsConnected())
                Say(p.group.color + p.name + ": &0" + message, p.opchat);
        }
        public void Connect()
        {
