...
namespace MCForge
{
    ...
    public class Level : IDisposable
    {
        ...
        public int zombieswon = 0;
        public int humanswon = 0;
        public int roundtime = 8;
        ...
        public bool Unload(bool silent = true, bool save = true)
        {
        ...
        public void Blockchange(Player p, ushort x, ushort y, ushort z, byte type, bool addaction = true)
        {
            ...
            try
            {
                ...
                if (!Block.AllowBreak(b) && !Block.canPlace(p, b) && !Block.BuildIn(b) && !p.modemode)
                {
                ...
                if (Owners == "")
                {
                    if (p.group.Permission < permissionbuild && (!inZone || !AllowBuild) && !p.referee)
                    {
                        p.SendBlockchange(x, y, z, b);
                        Player.SendMessage(p, c.red + "this map is not buildable - please do not break/build blocks");
                        return;
                    }
                }
