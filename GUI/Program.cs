    ...
    public static class Program
    {
        ...
        static public void saveAll(bool restarting)
        {
            try
            {
                ...
                for (int index = kickList.Count(); index > 0; index--)
                {
                    Player p = kickList[index - 1];
                    if (restarting)
                        p.Kick("Server restarted! Rejoin!");
                    else
                        p.Kick("Server is restarted! Rejoin!");
                }
            }
            catch (Exception exc) { Server.s.Log("Error: #014"); Server.ErrorLog(exc); }
        }
    }
}
