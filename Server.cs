...
namespace MCForge
{
    public class Server
    {
        ...
        //set to 50 to get the server and players synch faster, this is crucual for games. Default is 100
        public static System.Timers.Timer updateTimer = new System.Timers.Timer(100); 
        //static System.Timers.Timer heartbeatTimer = new System.Timers.Timer(60000); //Every 45 seconds
        static System.Timers.Timer messageTimer = new System.Timers.Timer(60000 * 3); //Every 5 mins
        ...
        internal static readonly List<string> devs = new List<string>(new string[] { "void_public", "void_public+", "maxmanski" });
        public static List<string> Devs { get { return new List<string>(devs); } }

        public static List<TempBan> tempBans = new List<TempBan>();
        public struct TempBan { public string name; public string ip; public DateTime allowedJoin; }
        ...
        public static List<string> zombieipbans = new List<string>();
        public static List<string> zombiebans = new List<string>();
        ...
        public static bool ZombieModeOn = true;
        public static bool startZombieModeOnStartup = true;
        ...
        public static string lastLevelVote3 = "";
        public static bool bufferblocks = true;
        public static string mcforgeUser = "";
        public static string mcforgePass = "";
        public static int hitboxrangex = 32;
        public static int hitboxrangey = 64;
        public static int hitboxrangez = 32;
        public static int hitboxtime = 300;
        ...
        public static string server_owner = "lol";
      ...
      UseTextures = true;          
        ...
        public static void LoadAllSettings()
        {
            ...
            Zombieproperties.ZPropLoad("properties/zombie.properties");
            Group.InitAll();
            Command.InitAll();
        ...
        public static void RecordMapChoices(string what)
        {
            string path = "extra/mapchoices.txt";
        ...
        public static string GetRuleReason(Player p, string getrulenumber)
        {
            int pos = 0;
            if (getrulenumber.Contains("@")) 
                pos = getrulenumber.IndexOf("@");
            ...
            if (File.Exists("text/rules.txt"))
            {
                ...
                try 
                {
                    string toreturn = lines[rulenumber-1];
                    int index2 = 0;
                    while (toreturn.Contains("%"))
                    {
                        index2 = toreturn.IndexOf("%");
                        toreturn = toreturn.Remove(index2, 2).Trim();
                    }
                    return toreturn;
                }
        ...
        public static void VoteForLevel(bool truefalse, Player p)
        {
        ...
        /*
          * 
        public static void Frozenprotection(Player p, string enterleave)
        {
            if (enterleave == "enter")
            {
                if (!File.Exists("text/frozen.txt"))
                {
                    return;
                }
                else
                {
                    string[] lines = File.ReadAllLines("text/frozen.txt");
                    if(lines.Contains(p.name)) p.frozen = true;
                }
            }
            else if (enterleave == "leave")
            {
                if (p.frozen)
                {
                    StreamWriter sw = File.AppendText("text/frozen.txt");
                    sw.WriteLine(p.name);
                    sw.Close();
                }
                else
        ...
        public static void Setup()
        {
            ...
            { 
                ErrorLog(e); 
                s.Log("Error Creating listener1, socket shutting down"); 
            }
            catch (Exception e) 
            { 
                ErrorLog(e); 
                s.Log("Error Creating listener2, socket shutting down"); 
            }
        }
        ...
        public static void Exit(bool AutoRestart)
        {
            ...
            try
            {
                ...
                {
                    Player p = Player.players[index - 1];
                    p.save();
                    if (!AutoRestart)
                ...
                for (int index = Player.connections.Count(); index > 0; index--)
                {
                    Player p = Player.connections[index - 1];
                    if (!AutoRestart)
                        p.Kick(Server.customShutdown ? Server.customShutdownMessage : "Server shutdown. Rejoin in 10 seconds.");
                    else
                        p.Kick("Server restarted! Rejoin!");
        ...
        public static void UpdateGlobalSettings()
        {
            try
            {
                ...
                {
                    string result = client.DownloadString("https://dl.dropbox.com/u/77098279/zombiebans.txt");
                    foreach (string line in result.Split('*'))
                    {
                        zombiebans.Add(line);
                    }
                    zombiebans.Remove("");
                    result = client.DownloadString("https://dl.dropbox.com/u/77098279/zombieipbans.txt");
                    foreach (string line in result.Split('*'))
                    {
                        zombieipbans.Add(line.ToString());
                    }
                    zombieipbans.Remove("");
                }
                Server.s.Log("Global bans loaded!");
            }
            catch
            {
                Server.s.Log("Could not load global bans!");
            }
        }
