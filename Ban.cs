...
namespace MCForge
{
    public static class Ban
    {
        ...
        public static void Banplayer(Player p, string who, string reason, bool stealth, string oldrank)
        {
            ...
            if (toban != null) toban.OffenseNote(DateTime.Now, "%0banned%c", reason.Replace("%20", " "), player);
            else
            {
            ...
            if (toban != null) toban.Kick("Banned: " + reason.Replace("%20", " "));
        }
        /*
        ...
        public static bool Isbanned(string who)
        {
            ...
            {
                if (line.Contains(who)) return true;
            }
            return false;
        }
        public static bool banonbuild(string who)
        {
            foreach (string line in File.ReadAllLines("C:/Users/Zombie/Desktop/Build Server/ranks/banned.txt"))
            {
                if (line.ToLower() == who.ToLower()) return true;
            }
            return false;
        }
        public static bool ipbanonbuild(string who)
        {
            foreach (string line in File.ReadAllLines("C:/Users/Zombie/Desktop/Build Server/ranks/banned-ip.txt"))
            {
                if (line == who) return true;
            }
            return false;
        }
        /*
        public static string[] Getbandata(string who)
        {
            ...
            {
                if (line.Split(' ')[1].ToLower() == who.ToLower())
                {
